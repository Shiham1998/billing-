...
createTimesetup(params: any) {
  // let id='';
  // return this.http.post<any>(`${baseUrl}/shiftmaster/add`, params).subscribe( data =>
  //   id = data.id
  // );
  
  
  return this.http.post(`${baseUrl}/productstock/add`, params);
  
  }
  updateProductstock(id: string, params: any) {
    return this.http.put(`${baseUrl}/productstock/update/${id}`, params);
  }

  deleteProductstock(id: string) {
    return this.http.delete(`${baseUrl}/productstock/delete/${id}`);
}
